...
type Sections struct {
	bonehealthtrackerMerge        string `json:"body_content"`
	bonehealthtrackerCommentMerge string `json:"body_comment_content"`
}
...
func SetCampaignContent(campaignId *CampaignId, template Template, token string) *CampaignContent {
	...
	resp, err := client.R().
		SetBody(Template{TemplateField: TemplateContent{
			...
			SectionsField: Sections{bonehealthtrackerMerge: template.TemplateField.SectionsField.bonehealthtrackerMerge, bonehealthtrackerCommentMerge: template.TemplateField.SectionsField.bonehealthtrackerCommentMerge},
		}}).
		SetHeader("Content-Type", "application/json").
